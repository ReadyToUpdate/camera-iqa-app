...
def build_dashboard(ws, results: list[ImageResult], input_folder: str | Path | None) -> None:
    ...
    ws.cell(metric_start + 1, 2, "平均值")
    ws.cell(metric_start + 1, 3, "最小值")
    ws.cell(metric_start + 1, 4, "最大值")
    ...
    for row_index, metric in enumerate(METRIC_COLUMNS, start=metric_start + 2):
        ...
        ws.cell(row_index, 2, round(sum(values) / len(values), 4) if values else "")
        ws.cell(row_index, 3, round(min(values), 4) if values else "")
        ws.cell(row_index, 4, round(max(values), 4) if values else "")
...
def build_samples(ws, results: list[ImageResult]) -> None:
    ws.append(["file", "severity", "defects", "preview"])
    row = 2
    for result in results:
        if not result.defects:
            continue
        ws.cell(row, 1, result.path.name)
        ws.cell(row, 2, result.severity)
        ws.cell(row, 3, result.defect_codes)
        if result.overlay_path and result.overlay_path.exists():
            try:
                ...
                ws.cell(row, 4, str(result.overlay_path))
        row += 1
    style_sheet(ws, freeze="A2")
